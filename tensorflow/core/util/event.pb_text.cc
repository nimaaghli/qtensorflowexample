...
void AppendProtoDebugString(
  ...
  if (msg.shutdown_mode() != 0) {
    o->AppendEnumName("shutdown_mode", ::tensorflow::EnumName_WorkerShutdownMode(msg.shutdown_mode()));
  }
...
bool ProtoParseFromScanner(
  ...
  while(true) {
    ...
    if (identifier == "shutdown_mode") {
      ...
      } else if (value == "SHUTDOWN_IMMEDIATELY" || value == "1") {
        msg->set_shutdown_mode(::tensorflow::SHUTDOWN_IMMEDIATELY);
      } else if (value == "WAIT_FOR_COORDINATOR" || value == "2") {
...
void AppendProtoDebugString(
  ...
  if (msg.health_status() != 0) {
    o->AppendEnumName("health_status", ::tensorflow::EnumName_WorkerHealth(msg.health_status()));
  }
...
bool ProtoParseFromScanner(
  ...
  while(true) {
    ...
    if (identifier == "health_status") {
      ...
      } else if (value == "RECEIVED_SHUTDOWN_SIGNAL" || value == "1") {
        msg->set_health_status(::tensorflow::RECEIVED_SHUTDOWN_SIGNAL);
      } else if (value == "INTERNAL_ERROR" || value == "2") {
