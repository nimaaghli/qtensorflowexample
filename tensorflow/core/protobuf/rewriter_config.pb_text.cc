...
void AppendProtoDebugString(
  ...
  if (msg.function_optimization() != 0) {
    o->AppendEnumName("function_optimization", ::tensorflow::EnumName_RewriterConfig_Toggle(msg.function_optimization()));
  }
  if (msg.debug_stripper() != 0) {
    o->AppendEnumName("debug_stripper", ::tensorflow::EnumName_RewriterConfig_Toggle(msg.debug_stripper()));
  }
  if (msg.meta_optimizer_iterations() != 0) {
    o->AppendEnumName("meta_optimizer_iterations", ::tensorflow::EnumName_RewriterConfig_NumIterationsType(msg.meta_optimizer_iterations()));
  }
...
bool ProtoParseFromScanner(
  ...
  while(true) {
    ...
    if (identifier == "layout_optimizer") {
      ...
      StringPiece value;
      if (!parsed_colon || !scanner->RestartCapture().Many(Scanner::LETTER_DIGIT_DASH_UNDERSCORE).GetResult(nullptr, &value)) return false;
      if (value == "DEFAULT_NUM_ITERS" || value == "0" || value == "-0") {
